 ...
 * 
 * @Override public List<NewModel> findAll(Pageble pageble) { StringBuilder sql
 * = new StringBuilder("SELECT * FROM news"); if (pageble.getSorter() != null &&
 * StringUtils.isNotBlank(pageble.getSorter().getSortName()) &&
 * StringUtils.isNotBlank(pageble.getSorter().getSortBy())) {
 * sql.append(" ORDER BY "+pageble.getSorter().getSortName()+" "+pageble.
 * getSorter().getSortBy()+""); } if (pageble.getOffset() != null &&
